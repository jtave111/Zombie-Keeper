...
const BOOT: BootLine[] = [
  { text: '[*] Initializing ZK C2 Framework v3.0.1...', color: 'var(--tx2)' },
  { text: '[*] Loading Spring Boot context...',          color: 'var(--tx2)' },
  ...
  { text: '[*] Awaiting operator credentials...',       color: 'var(--tx2)' },
];
...
export default function LoginPage({ onLogin }: Props) {
  ...
  return (
    <div style={{ height:'100vh', background:'var(--inset2)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Courier New' }}>
      <div style={{ display:'flex', width:'100%', maxWidth:920, height:540 }}>

        {/* LEFT */}
        <div style={{ width:420, background:'var(--inset2)', border:'1px solid #1a1a1a', borderRight:'none', display:'flex', flexDirection:'column' }}>
          <div style={{ background:'var(--inset2)', borderBottom:'1px solid #1a1a1a', padding:'5px 20px', display:'flex', justifyContent:'space-between', fontSize:10 }}>
            <span style={{ color:'var(--tx3)' }}>C2 INFRASTRUCTURE</span>
          ...
          <div style={{ flex:1, padding:'22px 26px', display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ background:'var(--inset2)', border:'1px solid #1a1a1a', padding:'14px', marginBottom:14 }}>
              <div style={{ fontSize:9, color:'var(--tx2)', letterSpacing:'1px', marginBottom:5, textTransform:'uppercase' }}>User</div>
              <input style={{ width:'100%', background:'var(--inset2)', border:'1px solid #1a1a1a', color:'#cccccc', fontFamily:'Courier New', fontSize:12, padding:'6px 8px', outline:'none', marginBottom:12, boxSizing:'border-box' as const }}
                placeholder="username" value={user} onChange={e=>setUser(e.target.value)}
                onKeyDown={e=>e.key==='Enter'&&handleLogin()} disabled={loading} />
              <div style={{ fontSize:9, color:'var(--tx2)', letterSpacing:'1px', marginBottom:5, textTransform:'uppercase' }}>Password</div>
              <input type="password" style={{ width:'100%', background:'var(--inset2)', border:'1px solid #1a1a1a', color:'#cccccc', fontFamily:'Courier New', fontSize:12, padding:'6px 8px', outline:'none', boxSizing:'border-box' as const }}
            ...
            <div style={{ flex:1, background:'var(--inset2)', border:'1px solid #111', overflow:'hidden', display:'flex', flexDirection:'column' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'4px 10px', background:'var(--inset2)', borderBottom:'1px solid #111' }}>
                <div style={{ display:'flex', gap:5 }}>
                  <div style={{ width:7, height:7, borderRadius:'50%', background:'rgba(224,92,110,0.5)' }}/>
                  <div style={{ width:7, height:7, borderRadius:'50%', background:'var(--inset)' }}/>
                </div>
                <span style={{ fontSize:9, color:'var(--tx3)', letterSpacing:1 }}>SESSION LOGS</span>
              </div>
        ...
        {/* RIGHT */}
        <div style={{ flex:1, background:'var(--inset2)', border:'1px solid #1a1a1a', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', position:'relative' as const }}>
          ...
          <div style={{ textAlign:'center', maxWidth:300 }}>
            ...
            <div style={{ fontSize:13, color:'var(--tx3)', fontStyle:'italic', lineHeight:1.9 }}>The puppeteer is</div>
            <div style={{ fontSize:13, color:'#e05c6e', fontStyle:'italic' }}>invisible.</div>
            <div style={{ height:10 }}/>
            <div style={{ fontSize:13, color:'var(--tx3)', fontStyle:'italic', lineHeight:1.9 }}>The puppet believes it</div>
            ...
            <div style={{ fontSize:9, color:'var(--tx3)', letterSpacing:2 }}>ZOMBIE KEEPER — C2 FRAMEWORK</div>
          </div>
          <div style={{ position:'absolute', bottom:12, fontSize:9, color:'#111', letterSpacing:1 }}>AUTHORIZED ACCESS ONLY</div>
          <div style={{ position:'absolute', top:10, right:12, fontSize:10, color:'var(--tx3)' }}>{time}</div>
        </div>
      </div>
    </div>
  );
