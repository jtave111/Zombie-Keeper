  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden', fontFamily:'Courier New' }}>
      ...
      <div style={{ padding:'8px 14px', background:'#0d0d0d', borderBottom:'1px solid #1a1a1a', display:'flex', alignItems:'center', gap:16, flexShrink:0 }}>
        <span style={{ fontSize:11, color:'#555', textTransform:'uppercase', letterSpacing:1 }}>Pivot &amp; Tunnel Manager</span>
      ...
      {/* Create form */}
      {creating && (
        <div style={{ padding:'10px 14px', background:'#0d0d0d', borderBottom:'1px solid #1a1a1a', display:'flex', gap:10, alignItems:'flex-end', flexShrink:0 }}>
          {[
            { label:'Type',        el: <select value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value as TunnelType}))} style={{background:'#040404',border:'1px solid #1e1e1e',color:'#ccc',fontFamily:'Courier New',fontSize:11,padding:'4px 8px',outline:'none',appearance:'none'}}>
              {(['SOCKS5','TCP-FWD','REV-FWD','DNS-TUNNEL'] as TunnelType[]).map(t=><option key={t}>{t}</option>)}</select> },
            { label:'Agent',       el: <select value={form.agent} onChange={e=>setForm(p=>({...p,agent:e.target.value}))} style={{background:'#040404',border:'1px solid #1e1e1e',color:'#ccc',fontFamily:'Courier New',fontSize:11,padding:'4px 8px',outline:'none',appearance:'none'}}>
              {['ZK-001','ZK-002','ZK-003','ZK-005','ZK-006'].map(a=><option key={a}>{a}</option>)}</select> },
            { label:'Local Port',  el: <input value={form.localPort} onChange={e=>setForm(p=>({...p,localPort:e.target.value}))} style={{width:80,background:'#040404',border:'1px solid #1e1e1e',color:'#ccc',fontFamily:'Courier New',fontSize:11,padding:'4px 8px',outline:'none'}}/> },
            { label:'Remote Host', el: <input value={form.remoteHost} onChange={e=>setForm(p=>({...p,remoteHost:e.target.value}))} placeholder="192.168.x.x" style={{width:130,background:'#040404',border:'1px solid #1e1e1e',color:'#ccc',fontFamily:'Courier New',fontSize:11,padding:'4px 8px',outline:'none'}}/> },
            { label:'Remote Port', el: <input value={form.remotePort} onChange={e=>setForm(p=>({...p,remotePort:e.target.value}))} style={{width:80,background:'#040404',border:'1px solid #1e1e1e',color:'#ccc',fontFamily:'Courier New',fontSize:11,padding:'4px 8px',outline:'none'}}/> },
          ].map(({label,el})=>(
            <div key={label}>
      ...
      <div style={{ flex:1, display:'flex', overflow:'hidden' }}>
        ...
        <div style={{ flex:1, overflowY:'auto' }}>
          {/* Table header */}
          <div style={{ display:'grid', gridTemplateColumns:'80px 90px 80px 100px 120px 90px 70px 70px 80px', padding:'4px 14px', background:'#111', borderBottom:'1px solid #1a1a1a', fontSize:9, color:'#333', textTransform:'uppercase', letterSpacing:0.8, flexShrink:0, position:'sticky', top:0 }}>
          ...
          {tunnels.map(t => (
            <div key={t.id} style={{ display:'grid', gridTemplateColumns:'80px 90px 80px 100px 120px 90px 70px 70px 80px', padding:'7px 14px', borderBottom:'1px solid #0d0d0d', alignItems:'center' }}>
              <span style={{ fontSize:10, color:'#555' }}>{t.id}</span>
              ...
              <span style={{ fontSize:10, color:'#444', fontFamily:'monospace' }}>:{t.localPort}</span>
              <span style={{ fontSize:10, color:'#333', fontFamily:'monospace' }}>{t.remoteHost ? `${t.remoteHost}:${t.remotePort}` : '—'}</span>
              <span style={{ fontSize:10, color:'#2a2a2a' }}>{t.bytes}</span>
              <span style={{ fontSize:10, color:'#2a2a2a' }}>{t.conns}</span>
              <div style={{ display:'flex', gap:4 }}>
                {t.status !== 'STOPPED' && (
                  <button onClick={()=>toggle(t.id)} style={{ background:'#080808', border:'1px solid #1a1a1a', color:t.status==='ACTIVE'?'#d48b55':'#33a84a', fontFamily:'Courier New', fontSize:9, padding:'2px 6px', cursor:'pointer' }}>
                    {t.status==='ACTIVE'?'STOP':'START'}
                  </button>
                )}
                <button onClick={()=>destroy(t.id)} style={{ background:'#080808', border:'1px solid #3d1520', color:'#e05c6e', fontFamily:'Courier New', fontSize:9, padding:'2px 6px', cursor:'pointer' }}>✕</button>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Chain diagram */}
        <div style={{ width:220, background:'#0a0a0a', borderLeft:'1px solid #1a1a1a', padding:'10px', overflow:'auto', flexShrink:0 }}>
          <div style={{ fontSize:9, color:'#2a2a2a', textTransform:'uppercase', letterSpacing:1, marginBottom:10 }}>Active Chain</div>
          <div style={{ fontFamily:'Courier New', fontSize:10, color:'#333', lineHeight:2 }}>
            <div style={{ color:'#e05c6e' }}>[ C2 Server ]</div>
            {activeChain.map((t,i) => (
              <div key={t.id}>
                <div style={{ color:'#1a1a1a' }}>    │</div>
                <div style={{ color:'#555' }}>{String.fromCharCode(9484)}{Array(3).fill('─').join('')}&gt; <span style={{ color:TYPE_COLOR[t.type] }}>{t.type}</span></div>
                <div style={{ color:'#555' }}>│ {t.agent} :{t.localPort}</div>
                {i < activeChain.length-1 && <div style={{ color:'#1a1a1a' }}>│</div>}
              </div>
            ))}
            {activeChain.length === 0 && <div style={{ color:'#1a1a1a' }}>no active tunnels</div>}
            <div style={{ color:'#1a1a1a', marginTop:4 }}>    │</div>
            <div style={{ color:'#2a2a2a' }}>{String.fromCharCode(9492)}{Array(3).fill('─').join('')}&gt; TARGET</div>
          </div>
        </div>
      </div>
    </div>
